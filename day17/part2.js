...
const parseGrid = (array) => {
  ...
  const maxFound = (height + height) * 9
  const meta = { width, height, minFound: maxFound };
...
const calcHeatLoss = (grid, pos, dir, loss, lineCount, steps, meta) => {
  ...
  if (pos.y === grid.length - 1 && pos.x === grid[0].length - 1 && lineCount >= 4) {
  ...
  for (const newDir of possibleDirs) {
    ...
    if (dir !== undefined && !sameDir && lineCount < 4) continue;
    if (sameDir && lineCount === 10) continue;
...
exec('sample.txt', 94, run);
exec('input.txt', 1283, run);
